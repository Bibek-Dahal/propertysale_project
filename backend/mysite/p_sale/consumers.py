...
from p_sale.models import House,Land

class KycStatusConsumer(AsyncWebsocketConsumer):
...
class IncreaseHouseViews(AsyncWebsocketConsumer):
    async def connect(self):
    ...
    def get_views_count(self,property_id):
        try:
            pro_obj = House.objects.get(id = property_id)
...
class IncreaseLandViews(AsyncWebsocketConsumer):
    async def connect(self):
        self.property_id = self.scope['url_route']['kwargs']['id']
        self.group_name = 'property_%d' % self.property_id
    ...
    def get_views_count(self,property_id):
        try:
            pro_obj = Land.objects.get(id = property_id)
            pro_obj.no_of_views += 1
            pro_obj.save()
            return pro_obj.no_of_views
        except Exception as e:
